...
js/
  app.js                ← Config, constants, utilities, platform switcher, init (~380 lines)
  cop15.js              ← COP15 Cascade module + Soak Timer (~2200 lines)
  testplan.js           ← Test Plan Manager module (~1549 lines)
  results.js            ← Results Analyzer module + Cpk/Ppk (~920 lines)
  inventory.js          ← Lab Inventory module (~1170 lines)
  firebase-sync.js      ← Optional Firebase cloud sync layer (~250 lines)
...
6. `firebase-sync.js` — Cloud sync (must be last, hooks into all save functions)
...
- Function naming: `tp*` = Test Plan, `ra*` = Results Analyzer, `inv*` = Inventory, `fb*` = Firebase sync, no prefix = COP15/shared
...
- **CDN dependencies**: signature_pad (for digital signatures), jsPDF (for PDF generation), Chart.js (for trend charts), and Firebase SDK (for optional cloud sync) are loaded from CDN in the head
...
- **Firebase Sync** is optional — enabled by filling `FIREBASE_CONFIG` in `js/firebase-sync.js`. Without config, app runs fully offline. Hooks wrap `saveDB()`, `tpSave()`, `raSave()`, `invSave()` to auto-push to Firestore on every save
- **Timeline compaction** — COP15 timeline entries store only summary data (operator, status, result) instead of full testData objects to reduce localStorage usage
