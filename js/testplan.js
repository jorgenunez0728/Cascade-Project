...
function tpGenerateWeekly() {
    ...
    for (const cfg of pool) {
        ...
        items.push({ desc:cfg.desc, id:cfg.id, mod:cfg.mod, rgn:cfg.rgn, reg:cfg.reg, eng:cfg.eng, tx:cfg.tx, my:cfg.my, drv:cfg.drv, body:cfg.body, ep:cfg.ep, engpkg:cfg.engpkg, tire:cfg.tire, required:cfg.required, deficit:cfg.deficit, score:cfg.score, completed:false, completedDate:null, manual:false, carriedOver:false, _scoreDetail: tpBuildScoreDetail(cfg, cfg.testedN, cfg.required, cfg.score) });
        used.add(cfg.desc);
    }
    ...
    if (lastAccepted) {
        lastAccepted.items.filter(function(i) { return !i.completed; }).forEach(function(i) {
            ...
            items.push({
                ...
                _scoreDetail: tpBuildScoreDetail(cfg, n, req, tpPriorityScore(cfg, n))
            });
    ...
    for (var i = 0; i < pool.length && items.length < capacity; i++) {
        ...
        items.push({
            ...
            _scoreDetail: { deficit: cfg.deficit, score: cfg.score, lastTested: _lastTested,
                reason: tpScoreReason(cfg, cfg.testedN) }
        });
        used.add(cfg.desc);
    }
...
function tpBuildFamilies() {
    ...
    Object.values(families).forEach(f => {
        ...
        f.overrideDeadline = _ov.deadline || '';
        f.daysToDeadline = _ov.deadline ? Math.ceil((new Date(_ov.deadline + 'T12:00:00') - Date.now()) / 86400000) : null;
            ...
            <details style="margin-bottom:4px;border:1px solid var(--tp-border);border-radius:8px;overflow:hidden;border-left:3px solid ${rc[_fRisk]};">
                ...
                <div style="padding:6px 8px;background:#0f1826;border-top:1px solid var(--tp-border);">
                    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:4px 6px;margin-bottom:6px;background:rgba(245,158,11,0.05);border:1px dashed rgba(245,158,11,0.3);border-radius:5px;">
                        ...
                        <label style="font-size:9px;color:var(--tp-dim);display:flex;align-items:center;gap:3px;">Deadline
                            <input type="date" value="${f.overrideDeadline||''}" class="tp-select" style="font-size:9px;padding:2px 4px;" onchange="tpSetFamilyOverride('${f.key.replace(/'/g,"\\'")}','deadline',this.value);">
                        </label>
                        ${f.overrideDeadline?`<button class="tp-btn tp-btn-ghost" onclick="tpSetFamilyOverride('${f.key.replace(/'/g,"\\'")}','deadline','');" style="font-size:8px;padding:1px 5px;color:var(--tp-red);">Quitar deadline</button>`:''}
                    </div>
