...
function invRenderDashboard(el) {
    ...
    var html = '<div style="display:flex;justify-content:flex-end;margin-bottom:8px;"><button class="tp-btn tp-btn-ghost" onclick="switchPlatform(\'panel\');if(typeof pnSwitchTab===\'function\')pnSwitchTab(\'pn-dashboard\');" style="font-size:10px;" title="Resumen cross-módulo del laboratorio">📊 Ver Resumen del Lab →</button></div>';
...
function invRenderEquipment(el) {
    ...
    html += '<button class="tp-btn tp-btn-primary" onclick="invAddEquipment()" style="font-size:10px;">+ Instrumento</button></div>';

    html += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(85px,1fr));gap:6px;margin-bottom:10px;">';
    html += '<div class="tp-metric"><div class="tp-metric-val" style="color:#10b981">' + summary.vigentes + '</div><div class="tp-metric-label">Vigentes</div></div>';
    html += '<div class="tp-metric"><div class="tp-metric-val" style="color:#f59e0b">' + summary.porVencer + '</div><div class="tp-metric-label">Por vencer ≤60d</div></div>';
    html += '<div class="tp-metric"><div class="tp-metric-val" style="color:#ef4444">' + summary.vencidos + '</div><div class="tp-metric-label">VENCIDOS</div></div>';
    ...
    if (criticalOverdue.length > 0) {
        ...
        html += '<div style="font-weight:800;color:#b91c1c;font-size:11px;margin-bottom:4px;">⚠️ ' + criticalOverdue.length + ' instrumento(s) CRÍTICO(S) con calibración vencida — identificar como NO OPERABLE</div>';
        criticalOverdue.forEach(function(e) {
            html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:3px 0;font-size:10px;color:#7f1d1d;">' +
                '<span>' + escapeHtml(e.name) + '</span>' +
                '<button class="tp-btn tp-btn-danger" onclick="invShowCalRegisterModal(\'' + e.id + '\')" style="font-size:9px;padding:3px 8px;">Calibrar</button></div>';
        });
        html += '</div>';
    }

    html += '<div style="margin-bottom:8px;"><input type="text" placeholder="Buscar instrumento, KMM, equipo..." value="' + escapeHtml(window._invEqSearch || '') + '" oninput="window._invEqSearch=this.value;_invDebouncedRender();" style="width:100%;padding:6px 8px;border:1px solid var(--tp-border);border-radius:6px;font-size:11px;"></div>';
    ...
    chips.forEach(function(c) {
        ...
        html += '<button class="tp-btn ' + (active ? 'tp-btn-primary' : 'tp-btn-ghost') + '" onclick="window._invEqFilter=\'' + c[0] + '\';_invDebouncedRender();" style="font-size:9px;padding:4px 8px;">' + c[1] + '</button>';
    });
    html += '</div>';
    ...
    if (groups.length === 0) {
        ...
        groups.forEach(function(g) {
            ...
            html += '<span style="font-size:9px;color:var(--tp-dim);">' + okCount + '/' + g.items.length + ' vigentes</span>';
            if (g.asset) html += '<button class="tp-btn tp-btn-ghost" onclick="event.stopPropagation();event.preventDefault();invAddAsset(\'' + g.asset.id + '\')" style="font-size:9px;" title="Editar equipo" aria-label="Editar equipo padre">✏️</button>';
            html += '</span></summary>';
            ...
            g.items.forEach(function(e) {
                ...
                html += '<div style="flex:1;min-width:150px;"><div style="font-weight:600;font-size:11px;">' + escapeHtml(e.magnitude || e.type || e.name) + '</div>';
                html += '<div style="font-size:9px;color:var(--tp-dim);">' + (e.kmmId ? 'KMM ' + escapeHtml(e.kmmId) + ' · ' : '') + (e.nextCalDate ? 'próxima ' + e.nextCalDate : 'sin fecha') + '</div></div>';
                html += '<div style="display:flex;gap:5px;align-items:center;">';
                html += '<span style="font-size:9px;padding:2px 6px;border-radius:4px;background:' + st.color + '20;color:' + st.color + ';">' + st.label + '</span>';
                if (e.requiresCal !== 'No') html += '<button class="tp-btn tp-btn-primary" onclick="invShowCalRegisterModal(\'' + e.id + '\')" style="font-size:9px;padding:4px 8px;">✅ Calibrado</button>';
                html += '<button class="tp-btn tp-btn-ghost" onclick="invEditEquipment(\'' + e.id + '\')" style="font-size:9px;" title="Editar" aria-label="Editar instrumento">✏️</button>';
                html += '</div></div>';
            });
            html += '</div></details>';
        });
...
function invShowCalRegisterModal(eqId) {
    ...
    modal.innerHTML = '<div style="max-width:380px;margin:60px auto;background:#fff;border-radius:14px;padding:20px;position:relative;">' +
        ...
        '<div style="font-size:11px;color:#64748b;margin-bottom:14px;">' + escapeHtml(eq.name) + (eq.magnitude ? ' — ' + escapeHtml(eq.magnitude) : '') + '</div>' +
        '<div style="display:flex;flex-direction:column;gap:10px;">' +
        '<div><label style="font-size:11px;color:#475569;font-weight:600;">Fecha</label><input id="inv-calreg-date" type="date" value="' + localToday() + '" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:6px;"></div>' +
        '<div><label style="font-size:11px;color:#475569;font-weight:600;">No. de Certificado</label><input id="inv-calreg-cert" value="" placeholder="Ej. T-2199-2026" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:6px;"></div>' +
        '<div><label style="font-size:11px;color:#475569;font-weight:600;">Proveedor</label><input id="inv-calreg-prov" value="' + escapeHtml(eq.calLab || '') + '" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:6px;"></div>' +
        '</div>' +
        '<div style="display:flex;gap:8px;margin-top:14px;">' +
...
function invAddEquipment(editId) {
    ...
    modal.innerHTML = '<div style="max-width:560px;margin:24px auto;background:#fff;border-radius:14px;padding:20px;position:relative;max-height:92vh;overflow-y:auto;">' +
        ...
        (isEdit ? '<button onclick="showConfirm(\'Eliminar instrumento?\',function(){invState.equipment=invState.equipment.filter(function(x){return x.id!==\x27' + editId + '\x27;});invSave();invRender();document.getElementById(\x27invModal\x27).style.display=\x27none\x27;},{title:\'Eliminar\',type:\'danger\',confirmText:\'Eliminar\'})" style="padding:10px;background:#dc2626;color:#fff;border:none;border-radius:8px;cursor:pointer;">Eliminar</button>' : '') +
        '<button onclick="document.getElementById(\x27invModal\x27).style.display=\x27none\x27" style="padding:10px;background:#e2e8f0;border:none;border-radius:8px;cursor:pointer;">Cancelar</button>' +
        '</div></div>';
    if (typeof cascadeInjectTooltips === 'function') cascadeInjectTooltips();
}
...
function invAddAsset(editId) {
    ...
    modal.innerHTML = '<div style="max-width:420px;margin:50px auto;background:#fff;border-radius:14px;padding:20px;position:relative;">' +
        ...
        '<div><label style="font-size:11px;color:#475569;display:flex;align-items:center;gap:6px;"><input type="checkbox" id="inv-asset-blocks" ' + (v('blocksTesting') ? 'checked' : '') + '> Si está en mantenimiento, bloquea pruebas</label></div>' +
        ...
        (isEdit ? '<button onclick="showConfirm(\'Eliminar equipo? Sus instrumentos quedaran sin equipo padre.\',function(){invState.assets=invState.assets.filter(function(x){return x.id!==\x27' + editId + '\x27;});invSave();invRender();document.getElementById(\x27invModal\x27).style.display=\x27none\x27;},{title:\'Eliminar\',type:\'danger\',confirmText:\'Eliminar\'})" style="padding:10px;background:#dc2626;color:#fff;border:none;border-radius:8px;cursor:pointer;">Eliminar</button>' : '') +
        '<button onclick="document.getElementById(\x27invModal\x27).style.display=\x27none\x27" style="padding:10px;background:#e2e8f0;border:none;border-radius:8px;cursor:pointer;">Cancelar</button>' +
        '</div></div>';
    if (typeof cascadeInjectTooltips === 'function') cascadeInjectTooltips();
}
...
function invRenderMaint(el) {
    ...
    if (overdue.length > 0) {
        html += '<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:10px;margin-bottom:10px;">';
        html += '<div style="font-weight:800;color:#b91c1c;font-size:11px;margin-bottom:6px;">⚠️ ' + overdue.length + ' mantenimiento(s) vencido(s)</div>';
        overdue.forEach(function(o) {
            ...
            html += '<div><div style="font-size:11px;font-weight:700;">' + escapeHtml(o.asset ? o.asset.name : '?') + ' — ' + escapeHtml(o.act.desc) + '</div>';
            html += '<div style="font-size:9px;color:#7f1d1d;">Vencido desde semana ' + o.lastWeek + ' (' + o.count + ' semana' + (o.count > 1 ? 's' : '') + ' sin registrar)</div></div>';
            html += '<button class="tp-btn tp-btn-danger" onclick="invMaintMarkDone(\'' + o.act.id + '\');invRender();" style="font-size:9px;padding:4px 10px;">✔ Hecho</button>';
            html += '</div>';
        });
        html += '</div>';
    }
    ...
    if (dueThisWeek.length === 0) {
        html += '<div style="text-align:center;padding:10px;color:var(--tp-dim);font-size:11px;">Sin mantenimientos programados esta semana.</div>';
    } else {
        dueThisWeek.forEach(function(d) {
            ...
            html += '<div><div style="font-size:11px;font-weight:700;">' + escapeHtml(d.asset ? d.asset.name : '?') + ' — ' + escapeHtml(d.act.desc) + '</div>';
            html += '<div style="font-size:9px;color:var(--tp-dim);">' + escapeHtml(d.act.responsible || '') + '</div></div>';
            html += '<div style="display:flex;gap:5px;">';
            html += '<button class="tp-btn tp-btn-primary" onclick="invMaintMarkDone(\'' + d.act.id + '\');invRender();" style="font-size:9px;padding:4px 10px;">✔ Hecho</button>';
            html += '<button class="tp-btn tp-btn-ghost" onclick="invShowMaintDetailModal(\'' + d.act.id + '\')" style="font-size:9px;">…con detalle</button>';
            html += '</div></div>';
        });
    }
    html += '</div>';
    ...
    html += '<div class="tp-metric"><div class="tp-metric-val" style="color:#10b981">' + compliance.doneOnWeek + '</div><div class="tp-metric-label">En semana</div></div>';
    ...
    if (compliance.byAsset.length > 0) {
        html += '<div style="overflow-x:auto;"><table style="width:100%;font-size:10px;border-collapse:collapse;"><thead><tr style="text-align:left;color:var(--tp-dim);"><th style="padding:4px;">Equipo</th><th style="padding:4px;">Plan</th><th style="padding:4px;">Real</th><th style="padding:4px;">%</th></tr></thead><tbody>';
    ...
    html += '<div style="margin-bottom:8px;"><label style="font-size:10px;color:var(--tp-dim);">Año: </label><select onchange="window._invMaintYear=parseInt(this.value);invRender();" style="padding:4px 8px;border:1px solid var(--tp-border);border-radius:6px;font-size:11px;">';
    for (var yy = year - 1; yy <= year + 1; yy++) html += '<option value="' + yy + '" ' + (yy === year ? 'selected' : '') + '>' + yy + '</option>';
    html += '</select></div>';
    if (matrix.length === 0) {
        html += '<div style="text-align:center;padding:14px;color:var(--tp-dim);font-size:11px;">Sin actividades activas. Agrega el catálogo abajo.</div>';
    } else {
        html += '<div style="overflow-x:auto;"><table style="border-collapse:collapse;font-size:9px;min-width:900px;">';
    ...
    html += '<div style="padding:8px;">';
    html += '<div style="text-align:right;margin-bottom:6px;"><button class="tp-btn tp-btn-primary" onclick="invAddMaintActivity()" style="font-size:10px;">+ Actividad</button></div>';
    (invState.maintActivities || []).forEach(function(a) {
        ...
        html += '<div><div style="font-size:11px;font-weight:700;">' + escapeHtml(asset ? asset.name : '?') + ' — ' + escapeHtml(a.desc) + '</div>';
        html += '<div style="font-size:9px;color:var(--tp-dim);">' + a.freq + ' · desde sem ' + a.startWeek + ' · ' + escapeHtml(a.responsible || '') + (a.active === false ? ' · INACTIVA' : '') + '</div></div>';
        html += '<button class="tp-btn tp-btn-ghost" onclick="invAddMaintActivity(\'' + a.id + '\')" style="font-size:9px;">✏️</button>';
        html += '</div>';
    });
    html += '</div></details>';
    ...
    html += '<div style="padding:8px;">';
    if (log.length === 0) {
        html += '<div style="text-align:center;padding:10px;color:var(--tp-dim);font-size:11px;">Sin mantenimientos registrados.</div>';
    } else {
        log.forEach(function(l) {
            ...
            html += '<div style="display:flex;justify-content:space-between;padding:5px 4px;border-bottom:1px solid var(--tp-border);font-size:10px;flex-wrap:wrap;gap:4px;">';
...
function invShowMaintDetailModal(activityId) {
    ...
    modal.innerHTML = '<div style="max-width:380px;margin:60px auto;background:#fff;border-radius:14px;padding:20px;position:relative;">' +
        ...
        '<div style="font-size:11px;color:#64748b;margin-bottom:14px;">' + escapeHtml(act.desc) + '</div>' +
        '<div style="display:flex;flex-direction:column;gap:10px;">' +
        '<div><label style="font-size:11px;color:#475569;font-weight:600;">Fecha</label><input id="inv-mdet-date" type="date" value="' + localToday() + '" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:6px;"></div>' +
        '<div><label style="font-size:11px;color:#475569;font-weight:600;">Horas</label><input id="inv-mdet-hours" type="number" step="0.5" min="0" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:6px;"></div>' +
        '<div><label style="font-size:11px;color:#475569;font-weight:600;">Comentarios</label><input id="inv-mdet-comments" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:6px;"></div>' +
        '</div>' +
        '<div style="display:flex;gap:8px;margin-top:14px;">' +
