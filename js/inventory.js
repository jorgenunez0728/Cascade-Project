...
// Los primeros sinónimos de cada campo son los encabezados LITERALES del F11 (los
// mismos que escribe invExportF11Calibracion). Agregar uno basta para reconocer una
// redacción nueva. Ojo: nada de palabras sueltas como "laboratorio" o "fecha" —
// el F11 tiene "Laboratorio (auto)" y la contención las empataría mal.
var INV_F11_IMPORT_FIELDS = {
    f11Id:       { label: 'No. (ID del F11)',     syn: ['no', 'no.', 'id f11', 'no f11', 'numero', 'num', 'consecutivo'] },
    kmmId:       { label: 'ID KMM',               syn: ['id kmm', 'kmm', 'kmm id', 'id interno', 'identificacion kmm'] },
    serialNo:    { label: 'No. Serie',            syn: ['no serie', 'numero de serie', 'no de serie', 'serie', 'serial', 'serial number', 'n s', 'ns'] },
    name:        { label: 'Descripción',          syn: ['descripcion magnitud calibrada', 'descripcion', 'magnitud calibrada', 'instrumento', 'magnitud'] },
    lastCalDate: { label: 'Última calibración',   syn: ['ultima calibracion', 'fecha de calibracion', 'fecha calibracion', 'fecha ultima calibracion', 'calibrado el', 'last calibration', 'calibration date'] },
    nextCalDate: { label: 'Próxima calibración',  syn: ['proxima calibracion auto', 'proxima calibracion', 'fecha proxima calibracion', 'vencimiento calibracion', 'fecha de vencimiento', 'next calibration', 'due date'] },
    calCertNo:   { label: 'No. Certificado',      syn: ['no certificado', 'numero de certificado', 'no de certificado', 'certificado', 'certificate', 'certificate no', 'cert'] },
    calLab:      { label: 'Proveedor',            syn: ['proveedor quien calibra', 'proveedor', 'quien calibra', 'laboratorio de calibracion', 'calibrador', 'supplier', 'vendor'] },
...
// primeras 25 filas (el F11 trae título, revisión y fecha arriba). Exige una
// columna de fecha o certificado y una de identidad; si no, no es la hoja.
function _invF11FindHeader(grid) {
    if (typeof _pnProjAutoMap !== 'function') return null;
    var best = null;
    for (var i = 0; i < Math.min((grid || []).length, 25); i++) {
        var row = grid[i] || [];
        var filled = row.filter(function(c) { return _invF11Cell(c) !== ''; }).length;
        if (filled < 2) continue;
        var map = _pnProjAutoMap(row, INV_F11_IMPORT_FIELDS);
...
/**
 * [v24.3] LA definición de qué cambiaría el Excel del F11 sobre `equipment`. PURA.
 * @returns {{ok, reason?, headerRow, map, updates:[{eq, row, before, after, nextFromSheet, nextCalc, via}],
 *            unmatched:[{rowNo, label}], ambiguous:[{rowNo, label, why}], older:[{eq, rowNo, before, after}],
 *            unchanged:number, total:number}}
 */
function invCalImportAnalyze(grid, equipment) {
    var res = { ok: false, headerRow: -1, map: {}, updates: [], unmatched: [], ambiguous: [], older: [], unchanged: 0, total: 0 };
    var hdr = _invF11FindHeader(grid);
    if (!hdr) { res.reason = 'No encontré una fila de encabezados del F11 (se necesita "No." o "ID KMM" o "No. Serie", y "Última calibración" o "No. Certificado").'; return res; }
    ...
    // ¿dd/mm o mm/dd? Se decide con los datos de las dos columnas de fecha.
    var dateSamples = [];
    for (var r0 = hdr.row + 1; r0 < grid.length; r0++) {
        dateSamples.push(col(grid[r0] || [], 'lastCalDate'), col(grid[r0] || [], 'nextCalDate'));
    }
    var dmy = typeof _pnProjDetectDMY === 'function' ? _pnProjDetectDMY(dateSamples) : true;
    var normDate = function(v) {
        if (typeof _pnNormDate === 'function') return _pnNormDate(typeof v === 'string' ? v.trim() : v, dmy);
        var s = _invF11Cell(v); return /^\d{4}-\d{2}-\d{2}$/.test(s) ? s : '';
    };

    // Índices de identidad: un id que aparece en más de un instrumento NO identifica.
    var countBy = function(fn) {
        var m = {};
        eqs.forEach(function(e) { var k = fn(e); if (k) (m[k] = m[k] || []).push(e); });
        return m;
    };
    var byF11 = countBy(function(e) { return _invF11Cell(e.f11Id).toUpperCase(); });
    var byKmm = countBy(function(e) { return _invF11RealId(e.kmmId).toUpperCase(); });
    var bySn = countBy(function(e) { return _invF11RealId(e.serialNo).toUpperCase(); });
    var byName = countBy(function(e) { return _invF11Key(e.name); });
    var used = {};

    for (var r = hdr.row + 1; r < grid.length; r++) {
        var row = grid[r] || [];
        if (!row.some(function(c) { return _invF11Cell(c) !== ''; })) continue;
        var f11 = _invF11Cell(col(row, 'f11Id')).toUpperCase();
        var kmm = _invF11RealId(col(row, 'kmmId')).toUpperCase();
        var sn = _invF11RealId(col(row, 'serialNo')).toUpperCase();
        var nm = _invF11Key(col(row, 'name'));
        var last = normDate(col(row, 'lastCalDate'));
        var nextSheet = normDate(col(row, 'nextCalDate'));
        var cert = _invF11Cell(col(row, 'calCertNo'));
        var prov = _invF11Cell(col(row, 'calLab'));
        var freq = _invF11Freq(col(row, 'calFreq'));
        if (!last && !cert && !nextSheet && !prov && !freq) continue;  // títulos, totales, notas
        res.total++;
        if (!f11 && !kmm && !sn && !nm) {
            // Trae fecha o certificado pero ningún identificador utilizable ('-' no es serie):
            // se DECLARA, no se salta en silencio.
            res.unmatched.push({ rowNo: r + 1, label: 'sin No., ID KMM ni serie' });
            continue;
        }
        var label = [f11, kmm, sn, _invF11Cell(col(row, 'name'))].filter(Boolean).join(' · ');

        var target = null, via = '', why = '';
        var pick = function(list, how) {
            if (!list) return false;
            if (list.length === 1) { target = list[0]; via = how; return true; }
            why = why || (how + ' repetido en ' + list.length + ' instrumentos');
            return false;
        };
        if (f11) {
            if (!pick(byF11[f11], 'No.') && /^\d+$/.test(f11)) {
                // "No." como consecutivo (1, 2, 3…) → C-001, C-002… del F11
                pick(byF11['C-' + ('00' + f11).slice(-3)], 'No. consecutivo');
            }
        }
        if (!target && kmm) pick(byKmm[kmm], 'ID KMM');
        if (!target && sn) pick(bySn[sn], 'No. Serie');
        if (!target && nm) pick(byName[nm], 'descripción');
        if (!target) {
            if (why) res.ambiguous.push({ rowNo: r + 1, label: label, why: why });
            else res.unmatched.push({ rowNo: r + 1, label: label });
            continue;
        }
        if (used[target.id]) { res.ambiguous.push({ rowNo: r + 1, label: label, why: 'otra fila ya apunta a ' + (target.f11Id || target.name) }); continue; }
        used[target.id] = true;

        var before = { lastCalDate: target.lastCalDate || '', nextCalDate: target.nextCalDate || '',
                       calCertNo: target.calCertNo || '', calLab: target.calLab || '', calFreq: target.calFreq || '' };
        // Una fecha del Excel más VIEJA que la de la app no retrocede el registro:
        // alguien ya capturó una calibración más reciente con "✅ Calibrado".
        if (last && before.lastCalDate && last < before.lastCalDate) {
            res.older.push({ eq: target, rowNo: r + 1, before: before, after: { lastCalDate: last } });
            continue;
        }
        ...
        var changed = ['lastCalDate', 'nextCalDate', 'calCertNo', 'calLab', 'calFreq'].some(function(k) { return after[k] !== before[k]; });
        if (!changed) { res.unchanged++; continue; }
        res.updates.push({ eq: target, rowNo: r + 1, before: before, after: after, via: via,
                           nextFromSheet: !!(nextSheet && after.nextCalDate === nextSheet),
                           nextCalc: nextCalc });
    }
    return res;
}
...
function invCalImportRenderPreview() {
    ...
    if (res.updates.length) {
        h += '<div style="overflow-x:auto;"><table class="u-cards" style="width:100%;border-collapse:collapse;font-size: var(--fs-sm);">'
           + '<thead><tr><th style="text-align:left;">Instrumento</th><th>Última calibración</th><th>Próxima</th><th>Certificado</th><th>Quedará</th></tr></thead><tbody>';
        res.updates.forEach(function(u) {
            ...
            h += '<tr style="border-top:1px solid var(--border);">'
               + '<td><strong>' + escapeHtml(u.eq.f11Id || '') + '</strong> ' + escapeHtml(u.eq.name || '') + '<div class="u-muted" style="font-size: var(--fs-xs);">fila ' + u.rowNo + ' · por ' + escapeHtml(u.via) + '</div></td>'
...
if (typeof HELP_TABS !== 'undefined') Object.assign(HELP_TABS, {
    ...
    'inv-equipment': { title: 'Equipos y Calibración (COP15-F11)', text: 'Instrumentos del laboratorio agrupados por equipo padre, con semáforo de calibración (60 días para "por vencer"). Un crítico vencido debe identificarse como NO OPERABLE.', tips: [
        ...
        '"📥 Actualizar desde Excel": sube el COP15-F11 actualizado (.xlsx o .csv). La app encuentra sola la hoja, empata por el No. del F11 y te muestra qué cambia antes de guardar. Una fecha más vieja que la de la app no se aplica.',
