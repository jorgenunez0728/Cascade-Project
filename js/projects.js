...
function _pnProjectGanttHTML(p) {
    ...
    if (!days.length) return '<div style="text-align:center;padding:20px;color:var(--tp-dim);font-size:11px;">Sin fechas capturadas todavía.</div>';
    ...
    html += '<button class="tp-btn tp-btn-ghost" onclick="pnProjectBaselineSet(\'' + p.id + '\')" style="font-size:11px;" data-help="pn-proj-baseline">' + (hasBaseline ? '🔄 Volver a fijar línea base' : '📌 Fijar línea base') + '</button>';
    html += '</div>';
    return html;
}
...
function _pnProjectKanbanHTML(p) {
    var steps = (p.steps || []).slice().sort(function(a, b) { return (a.seq || 0) - (b.seq || 0); });
    if (!steps.length) return '<div style="text-align:center;padding:20px;color:var(--tp-dim);font-size:11px;">Sin pasos. Agrega uno en la pestaña Tabla.</div>';
    var today = localToday();
    var cpm = pnProjectCPM(p);
...
function _pnProjectWorkloadHTML(p) {
    ...
    html += '</div>';
    if (!rows.length) return html + '<div style="text-align:center;padding:20px;color:var(--tp-dim);font-size:11px;">Sin pasos que repartir todavía.</div>';
...
function _pnProjectSCurveHTML(p) {
    ...
    if (!data) return '<div style="text-align:center;padding:20px;color:var(--tp-dim);font-size:11px;">Hace falta al menos un paso con fecha objetivo para dibujar la curva.</div>';
...
function _pnPortfolioHTML() {
    ...
    var html = '<div class="tp-card"><div class="tp-card-title" data-help="pn-proj-portfolio"><span>🗂️ Portafolio — ' + rows.length + ' proyecto' + (rows.length === 1 ? '' : 's') + ' activo' + (rows.length === 1 ? '' : 's') + '</span>' +
        '<button class="tp-btn tp-btn-ghost" onclick="pnExportPortfolioCSV()" style="font-size:10px;">📤 CSV</button></div>';
    ...
    rows.forEach(function(r) {
        ...
        html += '<td><strong>' + escapeHtml(p.name) + '</strong>' + (r.asset ? '<div style="font-size:9px;color:var(--tp-dim);">🔧 ' + escapeHtml(r.asset.name) + '</div>' : '') + '</td>';
        html += '<td>' + escapeHtml(p.owner || '—') + '</td>';
        html += '<td style="min-width:110px;"><div class="tp-bar" style="height:14px;"><div class="tp-bar-fill" style="width:' + r.prog.pct + '%;background:' + (r.prog.pct === 100 ? 'var(--tp-green)' : 'var(--tp-blue)') + ';"></div><span class="tp-bar-text" style="line-height:14px;font-size:9px;">' + r.prog.done + '/' + r.prog.total + '</span></div></td>';
        ...
        html += '<td>' + (r.nextMs ? '◆ ' + escapeHtml(r.nextMs.title) + '<div style="font-size:9px;color:var(--tp-dim);">' + r.nextMs.targetDate + '</div>' : '—') + '</td>';
        html += '</tr>';
    });
...
function pnAddProjectStep(projectId, stepId) {
    ...
    if (others.length) {
        ...
        depsHTML = '<div><label style="' + lblStyle + '" data-help="pn-proj-depends">Depende de (Ctrl+clic para varios)</label>' +
            ...
            '<div style="font-size:10px;color:var(--tp-dim);margin-top:3px;">Este paso no puede empezar hasta que los seleccionados terminen. Solo se listan los que no crean un círculo.</div></div>';
    }
    ...
    var msg = '<div style="display:flex;flex-direction:column;gap:10px;text-align:left;">' +
        ...
        '</div>' +
        '<details><summary style="cursor:pointer;font-size:11px;font-weight:700;color:#475569;padding:4px 0;">Más detalles (fase, obstáculo, inicio, hito, dependencias)</summary>' +
        '<div style="display:flex;flex-direction:column;gap:10px;padding-top:8px;">' +
        ...
        depsHTML +
        (s && s.baselineTarget ? '<div style="font-size:11px;color:var(--tp-dim);">📅 Comprometido originalmente: <strong>' + s.baselineTarget + '</strong>' +
